...
func main() {
	...
	sglangPort := flag.String("sglang-port", "8001", "the port SGLang is listening on")
	...
	// Choose target port based on connector
	targetPort := *vLLMPort
	if *connector == proxy.ConnectorSGLang {
		targetPort = *sglangPort
	}

	targetURL, err := url.Parse(scheme + "://localhost:" + targetPort)
	if err != nil {
